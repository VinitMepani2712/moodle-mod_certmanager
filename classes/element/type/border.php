...
class border extends base {
    ...
    public function add_form_fields(\MoodleQuickForm $mform, \context $context): void {
        $mform->addElement('text', 'thickness', 'Line thickness (mm)', ['size' => 5]);
        $mform->setType('thickness', PARAM_FLOAT);
        $mform->setDefault('thickness', $this->get_data_field('thickness', 1.0));
    }
    ...
    public function extract_data_from_form(\stdClass $data, \context $context): array {
        return ['thickness' => (float)($data->thickness ?? 1.0)];
    }
    ...
    public function render_pdf(
        ...
        $pdf->SetLineWidth(max(0.1, (float)$this->get_data_field('thickness', 1.0)));
        $w = $this->get_width() > 0 ? $this->get_width() : ($pdf->getPageWidth() - 10);
        $h = $this->get_height() > 0 ? $this->get_height() : ($pdf->getPageHeight() - 10);
        $pdf->Rect($this->get_posx(), $this->get_posy(), $w, $h);
    }
    ...
    public function render_html(\stdClass $certmanager, \stdClass $course): string {
        return '<div style="width:100%;height:100%;border:2px solid ' . s($this->get_colour()) . ';"></div>';
    }
}
