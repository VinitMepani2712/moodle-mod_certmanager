 ...
 * Logo element.
 *
 * A dedicated image element for an organisation / issuer logo. Behaves like
 * the generic "image" element but defaults to a top-centre position and a
 * logo-sized footprint, and appears as its own choice in the element picker.
 *
 * Uses the "elementfiles" file area, itemid = element id (unique per element,
 * so it never collides with background/image elements).
 */
class logo extends base {
    ...
    // Geometry [posx, posy, width, height] in mm; top-centre of a landscape A4 page.
    // height 0 => TCPDF scales proportionally from the width.
    /**
    ...
    public function render_pdf(
        ...
        try {
            ...
            debugging('mod_certmanager logo render failed: ' . $e->getMessage(), DEBUG_DEVELOPER);
        }
    ...
    public function render_html(\stdClass $certmanager, \stdClass $course): string {
        ...
        if (!$url) {
            return '<div class="certmanager-placeholder">[Logo not uploaded]</div>';
        }
        return '<img src="' . $url . '" style="width:100%;height:100%;object-fit:contain;" alt="Logo">';
    }
}
