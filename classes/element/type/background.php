...
/**
 * Full-page background image. Position/size are ignored — always fills the page.
 * Rendered before all other elements (special-cased by the renderer).
 */
class background extends base {
    ...
    public function render_pdf(
        ...
        try {
            ...
            debugging('mod_certmanager background render failed: ' . $e->getMessage(), DEBUG_DEVELOPER);
        }
    ...
    public function render_html(\stdClass $certmanager, \stdClass $course): string {
        ...
        if (!$url) {
            return '<div class="certmanager-bg-placeholder">Background (upload an image)</div>';
        }
        return '<img src="' . $url . '" style="width:100%;height:100%;object-fit:cover;" alt="">';
    }
}
